...
me.buildCalendar = (initFilters = {}, initRange = {}) => {
  ...
  const load = () => {
    ...
    return me
      ...
      .then(response => {
        ...
        response.results = response.results.map(order =>
          Object.assign(order, {
  ...
  api.nextMonth = () => {
    const nextMonthFrom = moment(range.dateFrom)
      .add(1, 'months')
      .toDate()
    const nextMonthTo = moment(range.dateTo)
      .add(1, 'months')
      .toDate()
...
me.getCalendar = ({ dateFrom, dateTo }) =>
  axios
    .get(`${API_URL}/rest/orders/calendar`, {
      params: {
        date_from: dateToParam(dateFrom),
        date_to: dateToParam(dateTo)
      }
...
me.getMenu = ({ date, menuId }) => {
  ...
  const promise = axios
    .get(`${API_URL}/rest/orders/menus/${menuId}`, {
      params: {
...
me.save = data => {
  return axios
    .post(`${API_URL}/rest/orders/${data.order_id ? data.order_id : ''}`, {
